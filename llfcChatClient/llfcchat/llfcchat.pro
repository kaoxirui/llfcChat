...
CONFIG(debug, debug|release) {
    TargetConfig = $$quote($${PWD}/config.ini)
    OutputDir = $$quote($${OUT_PWD}/$${DESTDIR})

    QMAKE_POST_LINK += echo "Copying config.ini to $$OutputDir" && cp "$$TargetConfig" "$$OutputDir/"

    StaticDir = $$quote($${PWD}/static)
    QMAKE_POST_LINK += $$QMAKE_COPY_DIR "$$StaticDir" "$$OutputDir/static/"
} else {
    TargetConfig = $$quote($${PWD}/config.ini)
    OutputDir = $$quote($${OUT_PWD}/$${DESTDIR})
    QMAKE_POST_LINK += $$QMAKE_COPY_FILE "$$TargetConfig" "$$OutputDir/"
